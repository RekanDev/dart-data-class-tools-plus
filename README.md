...
- Fixed LinkedMap type errors
- Improved DateTime serialization (ISO 8601 format)
- Fixed enum serialization (string names instead of indices)
- Fixed copyWith for sealed class subclasses
- Automatic const constructors
- Separated serialization methods in quick fixes
...
* `dart-data-class-generator.toString.useStringify`: If true and Equatable is used, generates `stringify` getter property instead of `toString()` method. If Equatable is not used, `toString()` will be generated instead.
